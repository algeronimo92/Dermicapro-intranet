 ...
 *
 * El sistema de verificación en el frontend/backend puede implementar:
...
/**
 * Genera todos los permisos base (access) para los módulos configurados
 */
export function generateBasePermissions() {
  return MODULE_CONFIG.map(config => ({
    name: getPermissionName(config.module, ACTIONS.ACCESS),
    displayName: `${config.icon} Acceso a ${config.displayName}`,
    description: `Permite acceder al módulo de ${config.description}`,
