        ...
        .then(({ body: services }) =>
          cy.request({
            ...
            body: {
              ...
              services: [{ serviceId: services[0].id, sessionNumber: 1 }],
            },
          })
        )
    );
...
describe('Atender cita', () => {
  it('el asistente puede iniciar la atención de una cita reservada', () => {
    cy.request('POST', '/api/auth/login', {
      ...
      createPatientAndAppointment(token, uniqueSuffix).then(({ body: appointment }) => {
        ...
        cy.get('.status-badge-large', { timeout: 8000 }).should('contain.text', 'En Atención');
      });
    });
  });
  ...
  it('el asistente puede finalizar la atención de una cita en progreso', () => {
    cy.request('POST', '/api/auth/login', {
      ...
      createPatientAndAppointment(token, uniqueSuffix).then(({ body: appointment }) => {
        // Poner la cita en in_progress via API
        cy.request({
          method: 'PUT',
          ...
          cy.loginAsAssistantAndVisit(`/appointments/${appointment.id}`);
          cy.get('.status-badge-large', { timeout: 6000 }).should('contain.text', 'En Atención');
          cy.contains('button', 'Finalizar Atención').click();
          cy.get('.status-badge-large', { timeout: 8000 }).should('contain.text', 'Atendida');
        });
      });
    });
  });
