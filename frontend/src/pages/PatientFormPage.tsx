...
import { PageFormLayout } from '../components/templates';
...
export const PatientFormPage: React.FC = () => {
  ...
  useEffect(() => {
    if (isEditMode && id) loadPatient(id);
  }, [id]);
  ...
  return (
    <PageFormLayout
      ...
      formActions={
        <>
          <Button type="button" variant="secondary" onClick={() => navigate('/patients')} disabled={isSaving}>
            Cancelar
          </Button>
    ...
    >
      <div className="form-grid">
        <div className="form-column">
          ...
          <Input label="Nombres *" name="firstName" value={formData.firstName} onChange={handleChange} error={errors.firstName} placeholder="Juan Carlos" />
          <Input label="Apellidos *" name="lastName" value={formData.lastName} onChange={handleChange} error={errors.lastName} placeholder="Pérez García" />
          <Input label="DNI *" name="dni" value={formData.dni} onChange={handleChange} error={errors.dni} placeholder="12345678" maxLength={8} />
          <DatePicker
        ...
        <div className="form-column">
          ...
          <Input label="Teléfono" name="phone" value={formData.phone} onChange={handleChange} error={errors.phone} placeholder="987654321" maxLength={9} />
          <Input label="Correo Electrónico" type="email" name="email" value={formData.email} onChange={handleChange} error={errors.email} placeholder="ejemplo@correo.com" />
          <Input label="Dirección" name="address" value={formData.address} onChange={handleChange} error={errors.address} placeholder="Av. Principal 123, Trujillo" />
        </div>
      </div>
    </PageFormLayout>
  );
